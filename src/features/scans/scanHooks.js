...
export function useScanDetails(scanId, auto = true) {
  ...
  const fetchOne = useCallback(async () => {
    ...
    try {
      ...
      setError(err?.message || "Failed to load scan");
      setScan(null); // ✅ prevent stale data
    } finally {
      setLoading(false);
    }
  }, [scanId]);
  ...
  useEffect(() => {
    if (auto && scanId) fetchOne();
  }, [auto, scanId, fetchOne]); // ✅ added scanId dependency
...
export function useUploadScan() {
  ...
  const upload = useCallback(async (file) => {
    ...
    try {
      ...
      const id = res?.scanId || res?.id;
      if (id) setScanId(id);

      return { ...res, scanId: id }; // ✅ normalize response
    } catch (err) {
      setError(err?.message || "Upload failed");
      return null;
    } finally {
